...
<head>
    ...
    <link rel="stylesheet" type="text/css" href="css/practitionerSearchSubjectResult.css">
        ...
        <?php
            ...
            echo "<div id=\"twoColContent\">";
                ...
                if(isset($_SESSION['username'])) {
                    ...
                    //query to obtain list of subjects belonging to logged in practitioner
                    $loggedInPractitioner = $_SESSION['username'];
                    $subjectListQuery = "SELECT SubjectID FROM PractitionerWithSub INNER JOIN Practitioner ON PractitionerWithSub.PractitionerID=Practitioner.PractitionerID WHERE Username = '$loggedInPractitioner';";
                    $subjectListResult = odbc_exec($conn, $subjectListQuery);
                    $count = 0;
                    while(odbc_fetch_row($subjectListResult)) {
                        $subjectIDForPractitioner[$count] = odbc_result($subjectListResult, "SubjectID");
                        $count += 1;
                    }
                    
                    //check how many matching search results
                    $count = 0;
                    for ($i = 0; $i < sizeof($subjectID); $i++) {
                        if(in_array($subjectID[$i], $subjectIDForPractitioner)) {
                            $count += 1;    
                        }
                    }
